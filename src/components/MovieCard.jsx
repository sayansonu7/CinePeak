...
const MovieCard = ({ movie, viewMode }) => {
        ...
        return (
            <div className="movie-card">
                ...
                <div className="movie-details">
                    ...
                    <p className="movie-info">Rating: {vote_average ? vote_average.toFixed(1) : 'N/A'}</p>
                    <p className="movie-info">Language: {original_language?.toUpperCase() || 'N/A'}</p>
    ...
    return (
        <div className="movie-card bg-gray-800 rounded-lg overflow-hidden shadow-lg hover:shadow-2xl transition-shadow duration-300 ease-in-out">
            <img src={posterUrl} alt={title} className="w-full h-auto object-cover" />
            <div className="p-4">
                <h3 className="text-lg font-bold text-white">{title}</h3>
                <div className="flex justify-between items-center mt-2">
                    <span className={`text-sm font-semibold ${vote_average >= 7 ? 'text-green-400' : 'text-yellow-400'}`}>
