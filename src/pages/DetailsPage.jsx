  ...
  try {
    ...
    for (const query of searchQueries) {
      ...
      if (searchData.releases && searchData.releases.length > 0) {
        bestRelease = searchData.releases.find(release => 
    ...
    if (bestRelease) {
      ...
      return {
        album: {
          ...
          artist: bestRelease['artist-credit'] ? bestRelease['artist-credit'][0].name : 'Various Artists',
          year: bestRelease.date ? new Date(bestRelease.date).getFullYear() : null,
          mbid: bestRelease.id,
          country: bestRelease.country || null
        },
        tracks: detailData.media && detailData.media[0] ? detailData.media[0].tracks : []
      };
    }
    return null;
  } catch (error) {
...
if (data && data.Title) {
        ...
        if (RAPID_API_KEY && data.imdbID) {
          ...
          try {
            ...
            navigator.geolocation.getCurrentPosition(
              ...
              (error) => {
                console.warn("Could not retrieve user location, defaulting to US data.", error); // User denied geolocation or an error occurred, fallback to 'us'
            ...
            async function fetchShowData(countryCode) {
              ...
              if (show && show.streamingOptions && show.streamingOptions[countryCode.toLowerCase()]) {
                const uniquePlatforms = Array.from(new Map(show.streamingOptions[countryCode.toLowerCase()].map(option => [
                  option.service.name,
                  {
            ...
            console.error("Error fetching streaming availability:", streamingError);
          }
        }
  ...
  useEffect(() => {
    if (movieDetails) {
      ...
      const isInWatchlist = watchlist.some(movie => movie.imdbID === movieDetails.imdbID);
  ...
  const handleWatchlistToggle = () => {
    ...
    if (isWatchlisted) {
      ...
      watchlist = watchlist.filter(movie => movie.imdbID !== movieDetails.imdbID);
      alert(`${movieDetails.Title} removed from watchlist!`);
    } else {
      // Add to watchlist
      watchlist.push({ imdbID: movieDetails.imdbID, Title: movieDetails.Title, Poster: movieDetails.Poster });
    ...
    <div className="details-page bg-gradient-to-br from-[#0f0c29] via-[#302b63] to-[#24243e]">
      ...
      <div className="relative h-screen w-full overflow-hidden">
        ...
        <div
          ...
          style={{
            backgroundImage: `url(${movieDetails.Poster && movieDetails.Poster !== 'N/A' ? movieDetails.Poster : ''})`,
        ...
        <div className="relative z-10 container mx-auto px-4 h-full flex flex-col md:flex-row items-center justify-center gap-8">
          ...
          <div className="w-72 shrink-0 rounded-lg shadow-2xl ring-1 ring-white/20 backdrop-blur-md bg-white/10 p-4 relative overflow-hidden">
            ...
            />
            {/* Faint inner glow */}
            <div className="absolute inset-0 rounded-lg pointer-events-none" style={{ boxShadow: 'inset 0 0 20px rgba(255,255,255,0.1)' }}></div>
          </div>

          {/* Movie Details */}
          <div className="flex-1 text-center md:text-left flex flex-col justify-center items-center md:items-start">
            {/* Title */}
            <h1 className={`text-5xl md:text-6xl font-extrabold text-white mb-4 leading-tight transition-opacity duration-1000 ${hasLoaded ? 'opacity-100' : 'opacity-0'}`}>
              {movieDetails.Title}
            </h1>
            {/* Pill Badges */}
            ...
            {/* Plot */}
            <p className="text-xl text-gray-200 max-w-prose leading-relaxed mb-8 text-center md:text-left">
              {movieDetails.Plot}
            </p>
            ...
            {movieDetails.imdbRating && movieDetails.imdbRating !== 'N/A' && (
              <div className="flex items-center gap-2 mb-6 p-2 rounded-full bg-gradient-to-r from-blue-500 to-purple-600  text-white shadow-lg">
                ...
                <span className="font-bold text-md px-1.5">{movieDetails.imdbRating}</span>
              </div>
            )}
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full max-w-sm">
              <a
                href={`https://www.youtube.com/results?search_query=${encodeURIComponent(movieDetails.Title + ' trailer')}`}
                target="_blank"
              ...
              >
                <Icon icon={isWatchlisted ? "mdi:check-circle" : "mdi:plus-circle"} className="inline-block w-5 h-5 mr-2" />
                ...
                {streamingPlatforms.length > 0 ? (
                  streamingPlatforms.map((platform) => (
                    ...
                    >
                      ...
                      />
                      <span className="text-lg text-gray-200 group-hover:text-white transition-colors text-center">
                        {platform.name}
                      </span>
                    </a>
                  ))
        ...
        {/* MusicBrainz Soundtrack Section */}
          <div className="mt-16">
            <h2 className="text-2xl font-semibold mb-8 flex items-center">
              <Icon icon="mdi:music-note" className="w-8 h-8 mr-3 text-blue-500" />
            ...
            <div className="bg-gray-800/50 rounded-xl p-6 backdrop-blur-sm">
              {soundtrackLoading ? (
                <div className="flex items-center justify-center py-12">
                  ...
                  <span className="ml-3 text-gray-300">Loading soundtrack...</span>
                </div>
              ) : soundtracks ? (
                <div>
                  {/* Album Header */}
                  <div className="flex items-start gap-6 mb-8 pb-6 border-b border-gray-700/50">
                    <div className="w-32 h-32 bg-gray-700/50 rounded-lg flex items-center justify-center overflow-hidden">
                      {movieDetails.Poster && movieDetails.Poster !== 'N/A' ? (
                        ...
                        <Icon icon="mdi:album" className="w-16 h-16 text-gray-200" />
                      )}
                    </div>
                    <div className="flex-1">
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center px-4 py-2 bg-purple-700 hover:bg-cyan-700 rounded-full text-white font-medium transition-colors duration-300"
                      >
                  ...
                  {soundtracks.tracks && soundtracks.tracks.length > 0 ? (
                    <div className="space-y-2">
                      <h4 className="text-lg font-semibold text-white mb-4">Track Listing</h4>
                      {(showAllTracks ? soundtracks.tracks : soundtracks.tracks.slice(0, 5)).map((track, index) => (
                        <div
                          key={track.id || index}
                        ...
                        >
                          <div className="flex items-center flex-1">
                            <span className="text-gray-400 text-sm w-8">
                              ...
                              <a
                                href={`https://www.youtube.com/results?search_query=${encodeURIComponent(track.title + ' ' + movieDetails.Title)}`}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="font-medium text-white group-hover:text-blue-400 transition-colors mr-2"
                              >
                                {track.title}
                              </a>
                              <a
                              ...
                              >
                                <Icon icon="simple-icons:youtube" className="w-4 h-4 text-red-500 hover:text-red-400" />
                              </a>
                            </div>
                            {track.recording && track.recording['artist-credit'] && (
                      ...
                      {soundtracks.tracks.length > 5 && (
                        <button
                          ...
                          className="mt-4 w-full py-2 px-4 bg-gray-600 hover:bg-blue-700 rounded-lg text-white font-medium transition-colors duration-300"
                        >
                    ...
                    <div className="text-center py-8">
                      <Icon icon="mdi:music-off" className="w-12 h-12 text-gray-600 mx-auto mb-4" />
                      <p className="text-gray-400 text-lg">No track listing available</p>
                      <p className="text-gray-500 text-sm mt-2">
                        Album found but track details are not available in the database
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="text-center py-12">
                  <Icon icon="mdi:music-off" className="w-16 h-16 text-gray-600 mx-auto mb-4" />
                  <p className="text-gray-400 text-lg mb-4">No official soundtrack found</p>
                  <p className="text-gray-500 text-sm mb-6">
                    Search for "{movieDetails.Title} soundtrack" on your preferred music platform
                  </p>
                  
                  {/* Fallback Search Links */}
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <a
                      href={`https://www.youtube.com/results?search_query=${encodeURIComponent(movieDetails.Title + ' soundtrack')}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex flex-col items-center p-4 bg-red-600/20 hover:bg-red-600/30 rounded-lg transition-colors"
                    >
                      <Icon icon="simple-icons:youtube" className="w-8 h-8 text-red-500 mb-2" />
                      <span className="text-sm text-white">YouTube</span>
                    </a>
                    
                    <a
                      href={`https://open.spotify.com/search/${encodeURIComponent(movieDetails.Title + ' soundtrack')}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex flex-col items-center p-4 bg-green-600/20 hover:bg-green-600/30 rounded-lg transition-colors"
                    >
                      <Icon icon="simple-icons:spotify" className="w-8 h-8 text-green-500 mb-2" />
                      <span className="text-sm text-white">Spotify</span>
                    </a>
                    
                    <a
                      href={`https://music.apple.com/search?term=${encodeURIComponent(movieDetails.Title + ' soundtrack')}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex flex-col items-center p-4 bg-gray-700/50 hover:bg-gray-600/50 rounded-lg transition-colors"
                    >
                      <Icon icon="simple-icons:applemusic" className="w-8 h-8 text-white mb-2" />
                      <span className="text-sm text-white">Apple Music</span>
                    </a>
                    
                    <a
                      href={`https://musicbrainz.org/search?query=${encodeURIComponent(movieDetails.Title + ' soundtrack')}&type=release`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex flex-col items-center p-4 bg-blue-600/20 hover:bg-blue-600/30 rounded-lg transition-colors"
                    >
                      <Icon icon="mdi:database" className="w-8 h-8 text-blue-500 mb-2" />
