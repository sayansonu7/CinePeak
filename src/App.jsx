...
const App = () => {
  ...
  useEffect(() => {
    const fetchAllPopularMovies = async () => {
      ...
      try {
        ...
        const popularMoviesDataPromises = popularMovieIds.map(async (id) => {
          ...
          try {
            ...
            if (data.Response === 'True') {
              ...
              console.warn(`OMDB API returned 'False' for popular movie ID ${id}: ${data.Error}`);
              return null;
            }
          } catch (error) {
        ...
        const popularMoviesData = (await Promise.all(popularMoviesDataPromises)).filter(movie => movie);
        setAllPopularMovies(popularMoviesData);
      } catch (error) {
        console.error(`Error fetching popular movies: ${error}`);
        setErrorMessage(`Error fetching popular movies: ${error.message}. Please try again later.`);
      } finally {
        setIsLoading(false);
      }
    };
    ...
    const fetchSearchedMovies = async (query) => {
      ...
      try {
        ...
        if (data.Response === 'False') {
          ...
          const detailedMoviesPromises = data.Search.map(async (searchResultMovie) => {
            ...
            try {
              ...
              if (!detailResponse.ok) {
                console.warn(`Failed to fetch details for movie ID ${searchResultMovie.imdbID}: ${detailResponse.statusText}`);
                return { ...searchResultMovie, imdbRating: "N/A", Language: "N/A" };
              }
              const detailData = await detailResponse.json();
              return detailData.Response === 'True' ? detailData : { ...searchResultMovie, imdbRating: "N/A", Language: "N/A" };
            } catch (error) {
              console.error(`Error fetching details for movie ID ${searchResultMovie.imdbID}:`, error);
              return { ...searchResultMovie, imdbRating: "N/A", Language: "N/A" };
            }
          });
        ...
        setErrorMessage(`Error fetching movies: ${error.message}. Please try again later.`);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const totalPages = debouncedSearchTerm ? 1 : Math.ceil(sortedList.length / MOVIES_PER_PAGE);
      ...
      <div className="wrapper">
        ...
        {isLoading && !moviesAvailable ? (
          ...
          <section className="movies-display">
            <div className="flex justify-between items-center mb-4">
              <div className="flex items-center">
                <h2>{debouncedSearchTerm ? `Search Results for "${debouncedSearchTerm}"` : `Popular Movies`}</h2>
              </div>
              <div className="flex items-center">
                <div className="view-mode-controls mr-4">
                  <button onClick={() => setViewMode('grid')} className={`px-3 py-1 border rounded ${viewMode === 'grid' ? 'bg-purple-700' : 'bg-gray-700'} text-white`}>Grid</button>
                  <button onClick={() => setViewMode('list')} className={`px-3 py-1 border rounded ${viewMode === 'list' ? 'bg-purple-700' : 'bg-gray-700'} text-white ml-2`}>List</button>
                </div>
                <div className="sorting-controls">
                <label htmlFor="sort-select" className="mr-2 text-sm text-gray-300">Sort by:</label>
                <select
                  id="sort-select"
                  value={sortCriteria}
            ...
            <div className={`${viewMode === 'grid' ? 'movie-grid' : 'movie-list'}`}>
              {listToDisplay.map((movie) => (
                <MovieCard key={movie.imdbID} movie={{
                  ...
                  original_language: movie.Language && movie.Language !== "N/A" ? movie.Language.split(',')[0].trim() : null,
              ...
              <div className="pagination-controls flex justify-center items-center space-x-4 my-8">
                ...
                {[...Array(totalPages).keys()].map(num => (
                    <button 
                        ...
                        className={`px-4 py-2 rounded ${currentPage === num + 1 ? 'bg-blue-900 text-white' : 'bg-blue-500 text-white hover:bg-blue-600'} transition-colors`}
                    >
                        {num + 1}
                    </button>
                ))}
          ...
          <p className="text-center py-4">
            {debouncedSearchTerm ? `No movies found for "${debouncedSearchTerm}".` : "No popular movies found."}
          </p>
        )}
      </div>
    </main>
